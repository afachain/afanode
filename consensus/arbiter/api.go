...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.arbiter.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetValidators(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.arbiter.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return nil, err
	}
...
func (api *API) Status() (*status, error) {
	...
	for n := start; n < end; n++ {
		...
		sealer, err := api.arbiter.Author(h)
		if err != nil {
			return nil, err
		}
		signStatus[sealer]++
