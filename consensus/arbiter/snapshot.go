...
type Snapshot struct {
	config   *params.ArbiterConfig // Consensus engine parameters to fine tune behavior
...
// the genesis block.
func newSnapshot(config *params.ArbiterConfig, sigcache *lru.ARCCache, number uint64, hash common.Hash, validators []common.Address) *Snapshot {
	snap := &Snapshot{
		config:     config,
		sigcache:   sigcache,
		Number:     number,
...
func loadSnapshot(config *params.ArbiterConfig, sigcache *lru.ARCCache, db ethdb.Database, hash common.Hash) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("arbiter-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("arbiter-"), s.Hash[:]...), blob)
}
