...
// Package arbiter implements the proof-of-staking consensus engine.
package arbiter
...
// Arbiter proof-of-stake-authority protocol constants.
var (
	...
	maxCrossChainTx = 10 // the max cross chain tx number for miner
)
...
var (
	...
	validatorsContractAddr = common.HexToAddress("0xAFA0000000000000000000000000000000000000")
	punishContractAddr     = common.HexToAddress("0xAFa0000000000000000000000000000000000001")
	proposalAddr           = common.HexToAddress("0xAFa0000000000000000000000000000000000002")
)
...
// Arbiter is the proof-of-stake-authority consensus engine proposed to support the
// Ethereum testnet following the Ropsten attacks.
type Arbiter struct {
	chainConfig *params.ChainConfig   // ChainConfig to execute evm
	config      *params.ArbiterConfig // Consensus engine configuration parameters
...
// New creates a Arbiter proof-of-stake-authority consensus engine with the initial
// validators set to the ones provided by the user.
func New(chainConfig *params.ChainConfig, db ethdb.Database) *Arbiter {
	// Set any missing consensus parameters to their defaults
	conf := *chainConfig.Arbiter
	if conf.Epoch == 0 {
...
func (a *Arbiter) SetStateFn(fn StateFn) {
	a.stateFn = fn
}

// Author implements consensus.Engine, returning the Ethereum address recovered
// from the signature in the header's extra-data section.
func (a *Arbiter) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
	// return ecrecover(header, a.signatures)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (a *Arbiter) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (a *Arbiter) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (a *Arbiter) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	// check extra data
	isEpoch := number%a.config.Epoch == 0
...
func (a *Arbiter) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (a *Arbiter) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number == 0 || (number%a.config.Epoch == 0 && (len(headers) > params.FullImmutabilityThreshold || chain.GetHeaderByNumber(number-1) == nil)) {
			...
			if checkpoint != nil {
				...
				snap = newSnapshot(a.config, a.signatures, number, hash, validators)
...
func (a *Arbiter) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (a *Arbiter) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (a *Arbiter) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (a *Arbiter) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	// If the block isn't a checkpoint, cast a random vote (good enough for now)
	header.Coinbase = a.validator
	...
	header.Difficulty = calcDifficulty(snap, a.validator)
	...
	if number%a.config.Epoch == 0 {
		newSortedValidators, err := a.getTopValidators(chain, header)
		if err != nil {
			return err
		}
...
// rewards given.
func (a *Arbiter) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header) error {
	// Initialize all system contracts at block 1.
	if header.Number.Cmp(common.Big1) == 0 {
		if err := a.initializeSystemContracts(chain, header, state); err != nil {
	...
	if header.Difficulty.Cmp(diffInTurn) != 0 {
		if err := a.tryPunishValidator(chain, header, state); err != nil {
			return err
		}
	}
	...
	if len(txs) > 0 {
		if err := a.trySendBlockReward(chain, header, state); err != nil {
			return err
		}
	}

	// do epoch thing at the end, because it will update active validators
	if header.Number.Uint64()%a.config.Epoch == 0 {
		newValidators, err := a.doSomethingAtEpoch(chain, header, state)
		if err != nil {
			return err
		}
...
func (a *Arbiter) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	// Initialize all system contracts at block 1.
	if header.Number.Cmp(common.Big1) == 0 {
		if err := a.initializeSystemContracts(chain, header, state); err != nil {
			panic(err)
	...
	// do epoch thing at the end, because it will update active validators
	if header.Number.Uint64()%a.config.Epoch == 0 {
		if _, err := a.doSomethingAtEpoch(chain, header, state); err != nil {
			panic(err)
		}
	}
...
func (a *Arbiter) trySendBlockReward(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	data, err := a.abi[validatorsContractName].Pack(method)
	if err != nil {
	...
	if _, err := executeMsg(msg, state, header, newChainContext(chain, a), a.chainConfig); err != nil {
		return err
	}
...
func (a *Arbiter) tryPunishValidator(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	number := header.Number.Uint64()
	snap, err := a.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
		return err
	...
	if !signedRecently {
		if err := a.punishValidator(outTurnValidator, chain, header, state); err != nil {
			return err
		}
	}
...
func (a *Arbiter) crossChain(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// ① 本地缓存跨链nonce
	// ② 读取该nonce之后的maxCrossChainTx条交易，（或许需要压缩结构体数组的方法，成为data，afa对该data进行解压,单困难造成多练不通用的风险，因为不一定支持压缩）
	// ③ 系统合约必须有记录目标链资产合约地址，与afa生成的链资产合约地址，如果没有则通过create创建
	// ④ 如上符合条件，则矿工节点发起交易执行转账，免除矿工的gas费用，实际上就是他自己的
	return nil
}

func (a *Arbiter) doSomethingAtEpoch(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) ([]common.Address, error) {
	newSortedValidators, err := a.getTopValidators(chain, header)
	if err != nil {
		return []common.Address{}, err
	}
	...
	if err := a.updateValidators(newSortedValidators, chain, header, state); err != nil {
		return []common.Address{}, err
	}
	//  decrease validator missed blocks counter at epoch
	if err := a.decreaseMissedBlocksCounter(chain, header, state); err != nil {
		return []common.Address{}, err
	}
...
func (a *Arbiter) initializeSystemContracts(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	snap, err := a.snapshot(chain, 0, header.ParentHash, nil)
	if err != nil {
		return err
	}
	...
	contracts := []struct {
		...
		{validatorsContractAddr, func() ([]byte, error) {
			return a.abi[validatorsContractName].Pack(method, genesisValidators)
		}},
		{punishContractAddr, func() ([]byte, error) { return a.abi[punishContractName].Pack(method) }},
		{proposalAddr, func() ([]byte, error) { return a.abi[proposalContractName].Pack(method, genesisValidators) }},
	}
...
func (a *Arbiter) getTopValidators(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
	parent := chain.GetHeader(header.ParentHash, header.Number.Uint64()-1)
	if parent == nil {
		return []common.Address{}, consensus.ErrUnknownAncestor
	}
	statedb, err := a.stateFn(parent.Root)
	if err != nil {
		return []common.Address{}, err
	}
	...
	// use parent
	result, err := executeMsg(msg, statedb, parent, newChainContext(chain, a), a.chainConfig)
	if err != nil {
		return []common.Address{}, err
	}

	// unpack data
	ret, err := a.abi[validatorsContractName].Unpack(method, result)
	if err != nil {
		return []common.Address{}, err
	}
	if len(ret) != 1 {
...
func (a *Arbiter) updateValidators(vals []common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// method
	method := "updateActiveValidatorSet"
	data, err := a.abi[validatorsContractName].Pack(method, vals, new(big.Int).SetUint64(a.config.Epoch))
	if err != nil {
...
func (a *Arbiter) punishValidator(val common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// method
	method := "punish"
	data, err := a.abi[punishContractName].Pack(method, val)
	if err != nil {
...
func (a *Arbiter) decreaseMissedBlocksCounter(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// method
	method := "decreaseMissedBlocksCounter"
	data, err := a.abi[punishContractName].Pack(method, new(big.Int).SetUint64(a.config.Epoch))
	if err != nil {
...
// with.
func (a *Arbiter) Authorize(validator common.Address, signFn ValidatorFn) {
	a.lock.Lock()
	defer a.lock.Unlock()
...
func (a *Arbiter) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	a.lock.RLock()
	val, signFn := a.validator, a.signFn
	a.lock.RUnlock()
	...
	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: val}, accounts.MimetypeArbiter, ArbiterRLP(header))
	if err != nil {
		return err
	}
...
func (a *Arbiter) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
	snap, err := a.snapshot(chain, parent.Number.Uint64(), parent.Hash(), nil)
	if err != nil {
		return nil
	}
	return calcDifficulty(snap, a.validator)
}
...
func (a *Arbiter) SealHash(header *types.Header) common.Hash {
	return SealHash(header)
}

// Close implements consensus.Engine. It's a noop for arbiter as there are no background threads.
...
func (a *Arbiter) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "arbiter",
		Version:   "1.0",
		Service:   &API{chain: chain, arbiter: a},
		Public:    false,
	}}
}
...
// ArbiterRLP returns the rlp bytes which needs to be signed for the proof-of-stake-authority
...
func ArbiterRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
