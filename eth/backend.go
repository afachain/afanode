...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/arbiter"
...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	// set state fn if consensus engine is arbiter.
	if arbiterEngine, ok := eth.engine.(*arbiter.Arbiter); ok {
		arbiterEngine.SetStateFn(eth.blockchain.StateAt)
	}
...
func (s *Ethereum) shouldPreserve(block *types.Block) bool {
	...
	// network is stuck.
	if _, ok := s.engine.(*arbiter.Arbiter); ok {
		return false
	}
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		if arb, ok := s.engine.(*arbiter.Arbiter); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
			}
			arb.Authorize(eb, wallet.SignData)
		}
